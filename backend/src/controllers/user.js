...
const UserController = (userModel, tokenService) => {
  ...
  //  message: String
  // }
  router.put('/:id', ownerMiddleware(tokenService, 'id'), async (req, res) => {
  ...
  // /user/:id DELETE
  //
  // Returns a status 204 no content on successful delete, otherwise will return
  // the appropriate error code, with the error under the body
  // {
  //  message: String
  // }
  router.delete('/:id', ownerMiddleware(tokenService, 'id'), async (req, res) => {
    const params = req.params;
    const id = parseInt(params.id, 10);
    const err = await userModel.deleteUser(id);
